...
Each tool is named `snake_case` of the spec `operationId` it calls, with the
direct-indexer `V1` suffix dropped (`fetchOrderBookV1` → `fetch_order_book`),
so the name an agent picks matches the operation and the other SDKs (R2.25,
ENG-17742). `scripts/check_spec_drift.py` enforces it.

The previous names (`list_markets`, `get_ticker`, `place_order`, …) stay
registered for one minor release as deprecated aliases: each forwards to the
same handler, and its description starts with ``Deprecated: use `<new name>`.``
The full map is `DEPRECATED_ALIASES` in
...
- **Registered withdrawal wallets** (v0.7.3, ENG-8902) —
  ...
  `confirm: true`.
- **Conditional order types** (v0.7.0) — `create_order` / `create_orders` /
...
(`create_ws_token` / `create_ws_token_legacy`) the caller uses to connect to them
...
`create_order`, `create_orders`, and `preview_order` (ENG-7550) — note the
...
are gone from it. `fetch_funding_samples` describes the series and never named
...
particular `fetch_portfolio_history`'s `limit` cap of 366 is that endpoint's alone
...
`cancel_all_orders`) require an explicit `confirm: true` flag so a stray call
can't do damage by accident.
...
`create_ws_token` and `create_ws_token_legacy` now return `ws_endpoint` alongside the
...
2. Ask: "Show me the BTC market on Nexus" — Claude calls `fetch_markets_summary` /
   `fetch_ticker` and reports the live BTC-USDX-PERP price.
