...
//   fetch_trading_fees       -> effective maker/taker bps, tier, 30d volume
//   fetch_balance            -> collateral snapshot
//   fetch_portfolio_history  -> equity + PnL + volume series over a window
//   fetch_equity_history     -> equity time-series (5s cadence, ~1h)
//   fetch_positions          -> open positions (with per-position risk detail)
//   fetch_positions_history   -> realized PnL per closed position
//   fetch_funding_history   -> funding paid/received
//   fetch_rate_limit_status  -> remaining request budget (agents: pace yourself)
//
...
async function main() {
  ...
  try {
    ...
    // fetch_account_summary with fetch_positions: the two parts can't disagree.
    const state = await callJson(client, "fetch_account_state");
    ...
    const summary = await callJson(client, "fetch_account_summary");
    console.log("\nPortfolio summary:");
    console.log(JSON.stringify(summary, null, 2).slice(0, 600));

    const fees = await callJson(client, "fetch_trading_fees");
    ...
    const equity = await callJson(client, "fetch_equity_history", {
      limit: 60,
    });
    ...
    // Longer horizon than fetch_equity_history, and with PnL + volume alongside
    // equity. This is a heavy read — one call, not a poll loop.
    const portfolio = await callJson(client, "fetch_portfolio_history", {
      window: "week",
    });
    ...
    const fundingPaid = await callJson(client, "fetch_funding_history", {
      limit: 5,
    });
    ...
    const rate = await callJson(client, "fetch_rate_limit_status");
    console.log("\nRate-limit status:");
